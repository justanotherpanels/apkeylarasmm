...
{
    /**
     * List available payment gateways for Mobile.
     */
    public function gateways(Request $request)
    {
        ...
        if ($paypal && !empty($paypal->api_config['client_id'])) {
            $available[] = [
                'name' => 'Paypal',
                'code' => 'paypal',
                'min_amount' => 1.00,
        ...
        if ($cryptomus && !empty($cryptomus->api_config['merchant_id'])) {
            $available[] = [
                'name' => 'Cryptomus',
                'code' => 'cryptomus',
                'min_amount' => 1.00,
    ...
    /**
     * Create a deposit transaction via Mobile API.
     */
    ...
    private function processPaypal(HistoryDeposit $deposit, $invoice, $amount, $user, Request $request)
    {
        ...
        try {
            ...
            // Determine redirect URLs (Mobile can override them)
            $returnUrl = $request->input('url_success') ?: ($request->input('url_return') ?: route('member.payment.paypal.callback'));
            $cancelUrl = $request->input('url_cancel') ?: route('member.payment.add');
    ...
    /**
     * Retrieve Deposit History for Mobile.
     */
